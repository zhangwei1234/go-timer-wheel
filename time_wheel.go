...
const(
		default_tick_duration = 100 * time.Millisecond //默认的时间轮 间隔时间 100毫秒
...
//添加一个超时任务
func (t *TimerWheel) AddTask(task Task, delay time.Duration) error {
	...
	err := t.scheduleTimeOut(timeOut)
	
	return err
}

func (t *TimerWheel) scheduleTimeOut(timeOut *WheelTimeOut) error{
